...
export function calculateBeastDamage(
  beast: Beast,
  itemLevel: number,
  itemTier: number,
  itemType: string,
  ...
  isNameMatch: boolean = false,
  equipment?: { neck?: { id: number, xp: number } },
  itemId?: number,
  itemSpecialsSeed?: number
): number {
  // Calculate base attack power from beast stats
  const baseAttack = beast.level * (6 - beast.tier);

  // Apply elemental adjustments to base attack
  let damage = baseAttack;
  if (isWeak) {
    damage = Math.floor(damage * 1.5); // 50% more damage when weak
  } else if (isStrong) {
    damage = Math.floor(damage * 0.5); // 50% less damage when strong
  }

  // Apply name match bonus
  if (isNameMatch && itemId !== undefined && itemSpecialsSeed !== undefined) {
    const itemSpecials = ItemUtils.getSpecials(itemId, itemLevel, itemSpecialsSeed);
  ...
  // Calculate armor value from item stats
  const armorValue = itemLevel * (6 - itemTier);

  // Subtract armor value
  damage = Math.max(2, damage - armorValue);

  // Apply amulet armor reduction if type matches
  if (equipment?.neck) {
    const neckLevel = Math.floor(Math.sqrt(equipment.neck.xp));
    const neckName = ItemUtils.getItemName(equipment.neck.id);

    // Check for specific neck item and armor type matches
    if ((itemType === 'Cloth' && neckName === 'Amulet') ||
      (itemType === 'Hide' && neckName === 'Pendant') ||
      (itemType === 'Metal' && neckName === 'Necklace')) {
      const neckReduction = Math.floor((armorValue * neckLevel * 3) / 100);
      damage = Math.max(2, damage - neckReduction);
    }
  }
...
export function calculateWeaponDamage(
  ...
  isStrong: boolean = false,
  isNameMatch: boolean = false,
  equipment?: { ring?: { id: number, xp: number } },
  itemId?: number,
  itemSpecialsSeed?: number,
  strength: number = 0,
): { baseDamage: number, criticalDamage: number } {
  // Calculate base attack power from weapon stats
  const baseAttack = itemLevel * (6 - itemTier);
  // Calculate beast armor value
  const beastArmor = beast.level * (6 - beast.tier);

  // Apply elemental adjustments to base attack
  let elementalAdjustedDamage = baseAttack;
  if (isWeak) {
    elementalAdjustedDamage = Math.floor(elementalAdjustedDamage * 1.5); // 50% more damage when weak
  } else if (isStrong) {
    elementalAdjustedDamage = Math.floor(elementalAdjustedDamage * 0.5); // 50% less damage when strong
  }

  // Calculate strength bonus
  let strengthBonus = 0;
  if (strength > 0) {
    strengthBonus = Math.floor((elementalAdjustedDamage * strength * 10) / 100);
  }
  ...
  if (isNameMatch && itemId !== undefined && itemSpecialsSeed !== undefined) {
    const itemSpecials = ItemUtils.getSpecials(itemId, itemLevel, itemSpecialsSeed);
    if (itemSpecials.suffix === beast.specialSuffix) {
      nameMatchBonus = elementalAdjustedDamage * 2; // Suffix match
    }
    if (itemSpecials.prefix === beast.specialPrefix) {
      nameMatchBonus = elementalAdjustedDamage * 8; // Prefix match
    }
  }

  // Calculate base damage (without critical)
  const baseDamage = Math.max(4, (elementalAdjustedDamage + strengthBonus + nameMatchBonus) - beastArmor);

  let criticalHitDamage = elementalAdjustedDamage;
  let critBonus = 0;
  // Titanium Ring gives 3% bonus per level on critical hits
  if (equipment?.ring && ItemUtils.getItemName(equipment.ring.id) === "TitaniumRing") {
    const ringLevel = Math.floor(Math.sqrt(equipment.ring.xp));
    critBonus += Math.floor((criticalHitDamage * 3 * ringLevel) / 100);
  }
  else if (equipment?.ring && ItemUtils.getItemName(equipment.ring.id) === "PlatinumRing" && isNameMatch) {
    const ringLevel = Math.floor(Math.sqrt(equipment.ring.xp));
    critBonus += Math.floor((nameMatchBonus * 3 * ringLevel) / 100);
  }

  let criticalDamage = Math.max(4, (elementalAdjustedDamage + strengthBonus + nameMatchBonus + criticalHitDamage) - beastArmor);
  criticalDamage += critBonus;
